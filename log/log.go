...
func New(option *Options) *zapLogger {
	if option == nil {
		option = NewOptions()
	}
	var info_log string
	var err_log string
	//if option.LogFileDir == ""{
	//	option.LogFileDir,_ = filepath.Abs(filepath.Dir(filepath.Join(".")))
	//	option.LogFileDir += sp + "logs" + sp
	//}
	if option.LogFileDir != "" {
		_, er := os.Stat(option.LogFileDir)
		if er != nil {
	...
	if err := zapLevel.UnmarshalText([]byte(option.Level)); err != nil {
		zapLevel = zapcore.InfoLevel
	}
	encodeLevel := zapcore.CapitalLevelEncoder
	// when output to local path, with color is forbidden
	if option.Format == consoleFormat && option.EnableColor {
	...
	loggerConfig := &zap.Config{
		...
		DisableCaller:     option.DisableCaller,
		DisableStacktrace: option.DisableStacktrace,
		...
		OutputPaths:      option.OutputPaths,
		ErrorOutputPaths: option.ErrorOutputPaths,
	}
